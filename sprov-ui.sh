...
install() {
    install_base
    bash <(curl -L -s https://github.com/dragonbbc/sprov-ui/raw/test/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
uninstall() {
    ...
    echo -e "Github issues: ${green}https://github.com/dragonbbc/sprov-ui/issues${plain}"
...
install_bbr() {
    bash <(curl -L -s https://github.com/dragonbbc/blog/raw/test/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/dragonbbc/sprov-ui/raw/test/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
